...
class AuthorizationWhitelistTest {
    ...
    private val contractWithWhitelistAdversaryAuth = "/checkers/whitelist/whitelisted-contract-adversary.fc"
    ...
    @Test
    fun `whitelist authorization adversary`() {
        val checker = extractCheckerContractFromResource(whitelistChecker)
        val contract = extractFuncContractFromResource(contractWithWhitelistAdversaryAuth)
        val checkerData = CellBuilder().storeUInt(0, 32).endCell()
        val tests =
            analyzeInterContract(
                listOf(checker, contract),
        ...
        tests.assertInvariantsHold(
            { test ->
                // as pc still contains TsaAccountIdSymbol even after rewrite
                (test.result.exitCode() == 1000) implies { test.resolvedAuthValues is AuthAnalysisResult.Unknown }
            },
        )
    }
...
fun TvmSymbolicTest.authorizedOwners(): List<TvmTestIntegerValue> = (this as TvmSymbolicTestFull).authorizedOwners()

fun TvmSymbolicTestFull.authorizedOwners(): List<TvmTestIntegerValue> =
