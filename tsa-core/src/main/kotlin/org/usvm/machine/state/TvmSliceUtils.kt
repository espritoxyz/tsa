...
private fun TvmContext.mkIteFromVariants(variants: List<Pair<UBoolExpr, UConcreteHeapRef>>): GuardedExpr<UHeapRef?> {
    if (variants.isEmpty()) {
        return GuardedExpr(null, falseExpr)
    }

    val guard =
        variants.fold(falseExpr as UBoolExpr) { acc, (guard, _) ->
            acc or guard
        }

    val value =
        variants.drop(1).fold(variants.first().second as UHeapRef) { acc, (guard, ref) ->
            mkIte(
                guard,
                trueBranch = ref,
                falseBranch = acc,
    ...
    return GuardedExpr(value, guard)
}
...
private fun TvmContext.processCellUnderflowCheck(
    cellRef: UHeapRef,
    getSize: (UHeapRef) -> UExpr<TvmSizeSort>,
    ...
    // cases for input and allocated cells are different

    // case of allocated
    if (allocated.expr != null) {
        val allocatedNoOverflow =
            scope.calcOnState {
                val size = getSize(allocated.expr!!)
                val min = minSize?.let { mkSizeGeExpr(size, minSize) } ?: trueExpr
                val max = maxSize?.let { mkSizeLeExpr(size, maxSize) } ?: trueExpr
                min and max
            }

        // Case of allocated cell: cellUnderflow is always a real error.
        scope.fork(
            allocated.guard implies allocatedNoOverflow,
    ...
    // case of input
    if (static.expr != null) {
        val staticNoOverflow =
            scope.calcOnState {
                val size = getSize(static.expr!!)
                val min = minSize?.let { mkSizeGeExpr(size, minSize) } ?: trueExpr
                val max = maxSize?.let { mkSizeLeExpr(size, maxSize) } ?: trueExpr
                min and max
            }

        var symbolicThrow = throwStructuralCellUnderflowError
...
fun TvmContext.checkCellDataUnderflow(
    ...
    val getCellSize = { ref: UHeapRef ->
        scope.calcOnState {
            fieldManagers.cellDataLengthFieldManager.readCellDataLength(this, ref)
        }
    }
    return processCellUnderflowCheck(cellRef, getCellSize, scope, minSize, maxSize, quietBlock)
}
    ...
    with(ctx) {
        val shiftedData =
            preloadDataBitsFromCellWithoutChecks(cell, offset, mkSizeExpr(sizeBits))
                ?: return@with null
        return@with mkBvExtractExpr(high = sizeBits - 1, low = 0, shiftedData)
    }
...
fun TvmStepScopeManager.slicePreloadDataBits(
    ...
    calcOnState {
        ...
        with(ctx) {
            ...
            val dataPosition = fieldManagers.cellDataLengthFieldManager.readSliceDataPos(state, slice)
...
fun TvmStepScopeManager.slicePreloadInt(
    ...
    val shiftedData =
        calcOnState { with(ctx) { slicePreloadDataBits(slice, sizeBits.extractToSizeSort(), quietBlock) } }
            ?: return null
...
fun TvmStepScopeManager.slicePreloadIntWithoutChecks(
    ...
    val shiftedData =
        calcOnState { with(ctx) { slicePreloadDataBitsWithoutChecks(slice, sizeBits.extractToSizeSort()) } }
            ?: return null
...
private fun TvmStepScopeManager.slicePreloadExternalAddrLengthConstraint(
    ...
    with(ctx) {
        if (!tvmOptions.enableExternalAddress && !mustProcessAllAddressFormats) {
            return@with falseExpr to zeroSizeExpr
        }
...
fun TvmStepScopeManager.slicePreloadNextRef(
    slice: UHeapRef,
    quietBlock: (TvmState.() -> Unit)? = null,
): UHeapRef? = calcOnState { with(ctx) { slicePreloadRef(slice, zeroSizeExpr, quietBlock) } }
...
fun TvmState.builderCopyFromBuilder(
    ...
    fieldManagers.cellRefsLengthFieldManager.writeCellRefsLength(
        this@builderCopyFromBuilder,
        result,
        fieldManagers.cellRefsLengthFieldManager.readCellRefLength(this@builderCopyFromBuilder, original),
    )
...
fun TvmStepScopeManager.builderCopy(
    ...
    with(ctx) {
        ...
        val cellData =
            cellDataFieldManager.readCellData(this@builderCopy, original)
                ?: return@with null
        doWithState {
            ...
            fieldManagers.cellRefsLengthFieldManager.writeCellRefsLength(
                this,
                result,
                fieldManagers.cellRefsLengthFieldManager.readCellRefLength(this, original),
            )
            copyCellRefs(original, result)
        }
    }
...
fun TvmStepScopeManager.builderStoreSlice(
    ...
    with(ctx) {
        ...
        doWithState {
            ...
            fieldManagers.cellRefsLengthFieldManager.writeCellRefsLength(memory, builder, resultingRefsSize)
        }
...
fun TvmStepScopeManager.builderStoreSliceWithForkOnOverflow(
    ...
    with(ctx) {
        ...
        val refsToWriteSize = cellRefsSize bvSub refsPosition
        val resultingRefsSize = oldBuilderRefsSize bvAdd refsToWriteSize
        val canWriteRefsConstraint = resultingRefsSize bvUle maxRefsLengthSizeExpr

        val oldBuilderDataLength =
            calcOnState {
                fieldManagers.cellDataLengthFieldManager.readCellDataLength(this, oldBuilder)
            }
        val newBuilderLength = mkSizeAddExpr(oldBuilderDataLength, bitsLeftInSlice)
        val canWriteBitsConstraint = mkSizeLeExpr(newBuilderLength, MAX_DATA_LENGTH.toSizeSort())

        val noOverflowCs = canWriteRefsConstraint and canWriteBitsConstraint
        val actions =
            listOf(
...
fun TvmState.getSliceRemainingRefsCount(slice: UHeapRef): UExpr<TvmSizeSort> =
    with(ctx) {
        val cell = memory.readField(slice, sliceCellField, addressSort)
        val refsLength =
            fieldManagers.cellRefsLengthFieldManager.readCellRefLength(this@getSliceRemainingRefsCount, cell)
...
private fun TvmStepScopeManager.tryCompareWithTlbFields(
    ...
    val cell1 =
        calcOnState {
            with(ctx) { memory.readField(slice1, sliceCellField, addressSort) as? UConcreteHeapRef }
        } ?: return null to Unit
    val cell2 =
        calcOnState {
            with(ctx) { memory.readField(slice2, sliceCellField, addressSort) as? UConcreteHeapRef }
        } ?: return null to Unit
            ...
            with(ctx) {
                val addrSlice =
                    if (tlbValue != null) {
                        ...
                        fieldManagers.cellRefsLengthFieldManager.writeCellRefsLength(memory, addrCell, addrRefPos)
...
fun builderStoreValueTlb(
    ...
    with(scope.ctx) {
        scope.doWithState {
            scope.storeCellDataTlbLabelInBuilder(builder, updatedBuilder, value, sizeBits)
        }
...
fun builderStoreSliceTlb(
    ...
    with(scope.ctx) {
        scope.builderStoreSlice(builder, updatedBuilder, slice, quietBlock)
            ?: return@with null

        scope.storeSliceTlbLabelInBuilder(builder, updatedBuilder, slice)
    }
...
fun TvmState.readCellDataLength(cell: CellRef): SizeExpr =
    fieldManagers.cellDataLengthFieldManager.readCellDataLength(this, cell.value)

fun TvmState.readCellDataLength(cell: UHeapRef): SizeExpr = readCellDataLength(cell.asCellRef())
