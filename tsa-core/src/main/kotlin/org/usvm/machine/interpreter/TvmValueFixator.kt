...
import org.usvm.machine.state.TvmFixationMemory
import org.usvm.machine.state.TvmFixationMemory.TvmFixationMemoryValues
...
class TvmValueFixator(
    ...
    private val emptyFixationValues = TvmFixationMemoryValues.empty(resolver.state.memory)
    ...
    ): Pair<UBoolExpr, TvmFixationMemoryValues>? = with(ctx) {
    ...
    private fun fixateConcreteValueForDataCell(
        ...
        val (childrenGuard, childrenValues) = value.refs.foldIndexed(
            (trueExpr as UBoolExpr) to emptyFixationValues
        ) { index, (accGuard, accValues), child ->
            val childRef = scope.calcOnState {
                readCellRef(ref, mkSizeAddExpr(mkSizeExpr(index), refsOffset))
            }
            val (curGuard, curValues) = fixateConcreteValue(scope, childRef, child)
                ?: return@with null
    ...
    private fun fixateConcreteValueForDictCell(
        ...
        val possibleEntries = scope.calcOnState {
        ...
        possibleEntries.forEach { entry ->
            ...
            if (concreteValue != null) {
                setKeys.add(model.eval(key) as KInterpretedValue<UBvSort>)

                val entryValue = scope.calcOnState {
                    dictGetValue(ref, dictId, key)
                }
                val (valueGuard, valueFixValues) = fixateConcreteValue(scope, entryValue, concreteValue)
                    ?: return@with null
            ...
            if (concreteValue == null) {
                resultGuard = resultGuard and keyContains.not() and keyConstraint
            } else {
                val valueConstraint = fixateConcreteValue(scope, entryValue, concreteValue)
                    ?: return@with null
                resultGuard = resultGuard and keyContains and valueConstraint and keyConstraint
            }
            */
        }

        val curSet = TvmFixationMemory.ConcreteSet(
            ref = modelRef,
            dictId = dictId,
            elements = setKeys,
        )

        val curFixValues = TvmFixationMemoryValues(
            setOf(curSet),
            nullRef = resolver.state.memory.nullRef(),
        )
