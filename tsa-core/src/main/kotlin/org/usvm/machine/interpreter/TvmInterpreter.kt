...
class TvmInterpreter(
    ...
    private val artificialInstInterpreter =
        TvmArtificialInstInterpreter(ctx, contractsCode, transactionInterpreter, tsaCheckerFunctionsInterpreter)
    ...
    fun getInitialState(
        ...
        val contractCode =
            contractsCode.getOrNull(startContractId)
        ...
        state.contractIdToInitialData =
            contractsCode.indices.associateWith {
                val c4 =
                    state.contractIdToC4Register[it]
                        ?: error("c4 for contract $it not found")
                val c7 =
                    state.contractIdToFirstElementOfC7[it]
    ...
    private fun visitBasicStackInst(
        scope: TvmStepScopeManager,
        stmt: TvmStackBasicInst,
    ) {
    ...
    private fun visitComplexStackInst(
        ...
        when (stmt) {
            ...
            is TvmStackComplexDepthInst -> {
                TODO("Cannot implement stack depth yet (TvmStackComplexDepthInst)")
            }

            is TvmStackComplexChkdepthInst -> {
                TODO("Cannot implement stack depth yet (TvmStackComplexChkdepthInst)")
            }

            is TvmStackComplexOnlytopxInst -> {
                TODO("??")
            }

            is TvmStackComplexOnlyxInst -> {
                TODO("??")
            }
        }
    ...
    private fun visitConstantIntInst(
        scope: TvmStepScopeManager,
        stmt: TvmConstIntInst,
    ) {
        scope.consumeDefaultGas(stmt)
        scope.doWithState {
    ...
    private fun TvmConstIntInst.bv257value(ctx: TvmContext): UExpr<TvmInt257Sort> =
        with(ctx) {
    ...
    private fun visitConstantDataInst(
        scope: TvmStepScopeManager,
        stmt: TvmConstDataInst,
    ) {
        when (stmt) {
    ...
    private fun visitPushContShortInst(
        scope: TvmStepScopeManager,
        stmt: TvmConstDataPushcontShortInst,
    ) {
        scope.doWithState {
    ...
    private fun visitArithmeticInst(
        scope: TvmStepScopeManager,
        stmt: TvmArithmBasicInst,
    ) {
        scope.consumeDefaultGas(stmt)
    ...
    private fun TvmContext.doSubtraction(scope: TvmStepScopeManager): UExpr<TvmInt257Sort>? {
        val secondOperand =
    ...
    private fun visitArithmeticLogicalInst(
        scope: TvmStepScopeManager,
        stmt: TvmArithmLogicalInst,
    ): Unit =
        with(ctx) {
            val result: UExpr<TvmInt257Sort> =
                when (stmt) {
    ...
    private fun visitComparisonIntInst(
        scope: TvmStepScopeManager,
        stmt: TvmCompareIntInst,
    ) = with(ctx) {
        scope.consumeDefaultGas(stmt)
    ...
    private fun TvmState.putBooleanAndToNewStmt(
        stmt: TvmInst,
        expr: UBoolExpr,
    ) {
    ...
    private fun visitComparisonOtherInst(
        scope: TvmStepScopeManager,
    ...
    private fun visitSliceDataEqInst(
        scope: TvmStepScopeManager,
        stmt: TvmCompareOtherSdeqInst,
    ) = with(ctx) {
        scope.consumeDefaultGas(stmt)
    ...
    private fun visitSaveInst(
        scope: TvmStepScopeManager,
        stmt: TvmContRegistersSaveInst,
    ) = scope.doWithState {
        scope.consumeDefaultGas(stmt)
        ...
        when (registerIndex) {
            ...
            3 -> {
                ...
                if (
                    ...
                    // we can extract TsaContractCode only if continuation has raw cell representation
                    cont.sourceCell == null
                ) {
    ...
    private fun visitTvmPushCtrInst(
        scope: TvmStepScopeManager,
        stmt: TvmContRegistersPushctrInst,
    ) {
        scope.consumeDefaultGas(stmt)
...
//            val data = registers.c4?.value?.value?.toSymbolic(scope) ?: mkSymbolicCell(scope)
    ...
    private fun visitCondsel(
        scope: TvmStepScopeManager,
        stmt: TvmContConditionalCondselInst,
    ) {
        scope.consumeDefaultGas(stmt)
    ...
    private fun visitIfElseInst(
        scope: TvmStepScopeManager,
        stmt: TvmContConditionalIfelseInst,
    ) {
        scope.consumeDefaultGas(stmt)

        scope.doWithState {
            val secondContinuation =
                stack.takeLastContinuation()
                    ?: return@doWithState ctx.throwTypeCheckError(this)
            val firstContinuation =
                stack.takeLastContinuation()
                    ?: return@doWithState ctx.throwTypeCheckError(this)
    ...
    private fun visitIfRefElseRefInst(
        scope: TvmStepScopeManager,
        stmt: TvmContConditionalIfrefelserefInst,
    ) {
        scope.doWithState {
    ...
    private fun visitIfRefElseInst(
        scope: TvmStepScopeManager,
        stmt: TvmContConditionalIfrefelseInst,
    ) {
        scope.doWithState {
            consumeGas(26) // TODO complex gas "26/126/51"
    ...
    private fun visitIfElseRefInst(
        scope: TvmStepScopeManager,
        stmt: TvmContConditionalIfelserefInst,
    ) {
        scope.doWithState {
            consumeGas(26) // TODO complex gas "26/126/51"
    ...
    private fun TvmStepScopeManager.doIf(
        ...
        val invertedCondition =
            (flag eq zeroValue).let {
...
//                        stack = continuation.stack
    ...
    private fun visitTvmDictionaryJumpInst(
        scope: TvmStepScopeManager,
        stmt: TvmContDictInst,
    ) {
        scope.consumeDefaultGas(stmt)
    ...
    private fun visitDictControlFlowInst(
        ...
        when (stmt) {
            is TvmDictSpecialDictigetjmpzInst -> {
                ...
                val method =
                    contractCode.methods[methodId] ?: run {
    ...
    private fun visitCodepageInst(
        scope: TvmStepScopeManager,
        stmt: TvmCodepageInst,
    ) {
        scope.consumeDefaultGas(stmt)
