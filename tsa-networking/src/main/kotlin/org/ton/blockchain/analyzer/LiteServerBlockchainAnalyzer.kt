...
class LiteServerBlockchainAnalyzer(
    ...
    override fun processLibraryCells(jettonContractInfo: JettonContractInfo): JettonContractInfo {
        val processedWalletCode =
            infoExtractor
                .extractOrdinaryCellIfLibraryCellGiven(jettonContractInfo.walletContractBytes)
                ?.base64ToHex()
    ...
    override fun extractLibraryCellByHashIfCan(libraryCellHash: ByteArray): ByteArray? =
        infoExtractor.getLibraryCellValueByHash(libraryCellHash)?.fromBase64()

    @OptIn(ExperimentalStdlibApi::class)
    override fun processStateWithPossibleLibraryCell(state: ContractState): ContractState {
        val processedWalletCode =
            infoExtractor
                .extractOrdinaryCellIfLibraryCellGiven(state.codeHex.hexToByteArray())
                ?.base64ToHex()
                ?: state.codeHex
